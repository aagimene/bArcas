...
0. **Rocker-relative station positions (model refactor).** Currently all station control points (`deckPt`, sheer `bottomPt`/`topPt`, `tip`, `deckEndPt`) are stored in absolute world X-Z. This means dragging the rocker only moves keel points; deck points and sheer pts stay in place, so every chord angle changes. The correct behaviour: moving the rocker should move the whole boat — all station pairs translate and rotate with the rocker frame at their attachment point, preserving their angle and height in the local frame.
   - **Interior stations:** store `deckLocal = {along, perp}` in the Frenet frame at `s` — `along` is the projection of the keel→deck chord onto the rocker tangent `t̂(s)`, `perp` is the projection onto the rocker normal `n̂(s)`. Derive world `deckPt = keel + along*t̂ + perp*n̂` on every render/loft. On deckPt drag: invert back to local. No change to the interaction model, only the storage.
...
1. **X-ordering constraint still broken — needs deeper investigation.** Three separate implementations (`x-ordering`, `x-bounds-fix`, `simple-bounds`) all lock the pink/sheer/tip control points in place. The constraint *in principle* is simple: each point's X must stay ≥ its immediate −X neighbour and ≤ its +X neighbour along its own control line. In practice some combination of the rocker-relative recompute (deckPt derived from deckLocal), the initial state geometry, or the Frenet-frame local storage is producing degenerate lo ≥ hi conditions before any drag has happened. Need to add diagnostic logging (console.log lo/hi/currX for each drag type) to see what values are actually in play before deciding the fix.

2. **Rotatable light in the 3D view (done ✓).** Old note about top-sheer CSS still applies if needed.
...
6. **Spine edge — closing and sharpness (refine the spine-radius concept).** The current spine-radius slider translates each half-mesh ±r in Y, which creates the right width but the two halves are open (not closed into a loop). Needs:
   - **Closing:** the keel/deck edge loop must be a closed surface — the two half-mesh edges at y=+r and y=−r need to be joined by a thin strip (or collapsed to a seam). Currently just offset open edges.
   - **Sharpness:** the "spine" cross-section shape should be configurable — from a rounded ellipse (large r, gentle) down to a knife edge (small r, near-zero thickness). Could be a separate "spine sharpness" or "bevel" parameter, or the shape of the closing strip itself.
   - Needs design work: how does this interact with the top/bottom hull split (item 7)? The split seam and the spine edge may need to share geometry. Defer implementation until the closing geometry and sharpness model are agreed.

 A single global radius `r` (default ~1 cm). Implementation note from user: do **not** scale the section by `r`. Instead, **translate** the entire starboard half-mesh outward in +Y by `r`, and the port half-mesh in −Y by `r`. The keel-centerline and deck-centerline edges (the b=0 vertices at top and bottom of every section) are no longer at Y=0 — they sit at ±r. This gives a tiny constant-width "spine" loop running around the deck-line and keel-line of the hull (a thin flat band, not a tapered tube). Slider range probably 0–0.05 m, default 0.01 m. Affects the 3D mesh only; the 2D editor panes unchanged.

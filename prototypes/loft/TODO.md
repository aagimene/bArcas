...
| Section view height no longer rescales while dragging control points: removed `maxB` from the SECTION_SCALE_N formula so the viewBox height is driven only by the hull's real H/B aspect ratio (Hm/Bm), not by the current max b among control points. Eliminates the feedback loop where dragging the widest point caused the displayed height to jump, shifting SVG coordinates mid-drag and accelerating the point. | stable-section |

...
| Section view scale frozen during drag: SECTION_SCALE_N (which sets the viewBox height to match the hull's H/B aspect) is no longer recomputed while sectionDrag is active. The scale still updates correctly at rest and on pointer-up; freezing it during drag prevents the viewBox from shifting SVG coordinates mid-drag, which was causing the feedback loop where moving the widest point made the view zoom uncontrollably. | section-scale-freeze |

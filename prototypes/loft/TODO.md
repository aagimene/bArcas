...
| Chine editor panel removed; chine interaction gated solely by the chines layer toggle per view. Feather handles, anchor drag in side/top (moves n/b along station curve), and click-on-anchor enters extend mode (chineDraw extended from existing stationIdxs). Hover ghost preview (chineHoverCursor) rendered in both side/top views when chines layer on and not drawing. Right-click on chine anchor in side/top triggers smart-delete same as section view. | inline-units |
| Hull length / beam width replaced with inline number inputs: ft + in + m for length (all editable), in + cm for beam (no ft). Editing any field updates the slider and all other unit fields. Slider still works as before. | inline-units |
| Keel/deck add-cursor: side view shows crosshair cursor + a blue circle preview at the nearest point on the rocker/deck curve when within click distance (0.12m threshold), matching existing click-to-add behavior. | inline-units |
| New station seeded from nearest neighbor (deep copy): addStationAtS now deep-copies the closest existing station's points array instead of calling sectionAtS. The new station inherits the exact control-point count, b/n values, handle angles/lengths, and chine assignments of the nearest neighbor, so insertion causes no visible geometry change — same behavior as sliding that neighbor to the new longitudinal position. sectionAtS is kept as fallback when no stations exist. | station-copy-nearest |
...
| View-settings (viz) panels: ⚙ button on every pane title opens a fixed-position popover for that view's settings. Top/Side/Section panels hold the reference-image controls (load, opacity, clear) moved out of the controls drawer; 3D panel holds render mode + color pickers + matcap/checker sub-options + wireframe overlay controls. Hull length and beam width sliders extended to full physical range (1 ft – 40 ft / 1 in – 60 in) with a free-text unit entry field ("16ft", "22in", "5m", etc.) that parses and drives the slider. | viz-config |
